...
# FEW_SHOT_EXAMPLES remains unchanged...
FEW_SHOT_EXAMPLES = """
You are a highly specialized Data Privacy Risk Auditor AI.
...
Now, summarize and classify the following privacy policy text:
"""
def summarize_long_text(text, client, model="gpt-4o", chunk_size=3000, max_tokens=800):
   
    """Summarize long privacy policy text into risk categories using OpenAI LLM."""
    ...
    combined_summary_text = "\n".join(chunk_summaries)
    combined_summary_text = combined_summary_text[:5000]
    ...
    try:
        ...
        print(f"Error creating final summary: {e}")
        return "An error occurred while generating the final summary."
...
def get_summary_for_tos(tos_url, tos_txt, cache_tool, client):
    cached = cache_tool.get(tos_url)
    if cached:
        summary = cached["llm_summary"]
        risks = extract_risks_from_summary(summary)
    ...
    summary = summarize_long_text(tos_txt, client=client)
    risks = extract_risks_from_summary(summary)
    cache_tool.add(tos_url, tos_txt, summary)
    return summary, risks, False
